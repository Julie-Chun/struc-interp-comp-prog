...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if place.ant:
                if isinstance(place.ant, ContainerAnt) and isinstance(self,Ant) and place.ant.contained_ant is None and not isinstance(self,ContainerAnt):
                    Insect.add_to(self, place)
                    place.ant.contain_ant(self)
                elif isinstance(place.ant, Ant) and isinstance(self, ContainerAnt) and self.contained_ant is None and not isinstance(place.ant,ContainerAnt):
                    Insect.add_to(self, place)
                    holder = place.ant
                    place.ant = self    # Ant become ContainerAnt
                    self = holder       # ContainerAnt becomes Ant
                    place.ant.contain_ant(self)
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while self.pl:
            if self.min_range <= index and index <= self.max_range:
                if self.pl is beehive:
                    return None
                else:
                    if self.pl.bees:    
                        return random_or_none(self.pl.bees)
            self.pl = self.pl.entrance 
            index += 1
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 7
        copy_bees = gamestate.bees
        for b in copy_bees:
            if b.place is self.place:
                b.reduce_armor(self.damage)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if not contained_ant and isinstance(other, ContainerAnt):
            return True
        return False
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.name == 'Imposter':
            ...
            while pl != None:
                if pl.ant:
                    if (pl.ant not in self.ants) and (pl.ant in gamestate.ants):
                        pl.ant.damage *= 2
                        self.ants += [pl.ant]
                    if isinstance(pl.ant, ContainerAnt) and pl.ant.contained_ant:
                        if pl.ant.contained_ant not in self.ants: 
                            pl.ant.contained_ant.damage *= 2
                            self.ants += [pl.ant.contained_ant]
                pl = pl.exit
    ...
    def remove_from(self, place):
        if self.place.ant is self and self.place.ant.name is 'Imposter':
            self.place.ant = None
            Insect.remove_from(self, place)
        elif isinstance(self.place.ant,ContainerAnt):
            if self.place.ant.contained_ant.name is 'Imposter':
                self = self.place.ant
                self.place.ant.contained_ant.place = None
                self.place.ant.contained_ant = None
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.direction[len(self.direction) - 1] == -1:
...
def apply_effect(effect, bee, duration):
    ...
    def applied(gamestate):
        ...
        if duration > 0:
            ...
            duration -= 1
            print("DEBUG: duration", duration)
            return effect(original, bee)(gamestate)
        bee.direction += [1]
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target not in target.bees_scared:
            target.bees_scared += [target]
