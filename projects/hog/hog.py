...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    Boo = False
    combo = 3 ** (player_score + opponent_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            point0 = take_turn(strategy0(score0,score1), score1, dice)
            score0 += point0
            who = other(who)
        else:
            strat1 = strategy1(score1,score0)
            point1 = take_turn(strategy1(score1,score0), score0, dice)
            score1 += point1
            who = other(who)

        if feral_hogs:
            if who == 1:
                if abs(strat0-prev_point0) == 2:
                    score0 += 3
                prev_point0 = point0
            elif who == 0:
                if abs(strat1 - prev_point1) == 2:
                    score1 += 3
                prev_point1 = point1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    num = 0
    while rolls < 11:
        exper = make_averaged(roll_dice, num_samples)(rolls,dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    possible = is_swap(bacon_score, opponent_score)
    beneficial = (opponent_score > bacon_score)
    alm = (bacon_score - score) >= margin
    detrimental = bacon_score > opponent_score
    
    if possible and beneficial:
        return 0
    elif possible and detrimental:
        return num_rolls
    elif alm:
        return 0
    else:
