...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if type(rest) is Pair:
            return scheme_apply(pro, rest.map(lambda items: scheme_eval(items, env)), env)
        else :
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        if symbol in list(self.bindings.keys()):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals == nil:
            ...
            return new.make_child_frame(formals.rest, vals.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        elif isinstance(expressions.rest, Pair):
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    if type(expressions.first) is int or type(expressions.first) is str or type(expressions.first) is Pair:
