...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return (length(left(m)) * left_weight == length(right(m)) * right_weight) and balanced(end(left(m))) and balanced(end(right(m)))
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, attempt):
    	...
    	if lives == 0:
    		return "Your account is locked. Attempts: " + str(password_attempts)
    	if attempt == password:
    		if amount > balance:
